...
=end
  ...
  def pbRefresh
    ...
    if @searchResults
      textpos.push([_INTL("Resultados"), 112, 314, 2, base, shadow])
      textpos.push([@dexlist.length.to_s, 112, 346, 2, base, shadow])
    else
      textpos.push([_INTL("Avistados:"), 42, 314, 0, base, shadow])
      textpos.push([$player.pokedex.seen_count(pbGetPokedexRegion).to_s, 182, 314, 1, base, shadow])
      textpos.push([_INTL("Obtenidos:"), 42, 346, 0, base, shadow])
  ...
  def pbRefreshDexSearch(params, _index)
    ...
    # Write various bits of text
    textpos = [
      [_INTL("Modo de búsqueda"), Graphics.width / 2, 10, 2, base, shadow],
      [_INTL("Orden"), 136, 64, 2, base, shadow],
      [_INTL("Nombre"), 58, 122, 2, base, shadow],
      [_INTL("Tipo"), 58, 174, 2, base, shadow],
      [_INTL("Altura"), 58, 226, 2, base, shadow],
      [_INTL("Peso"), 58, 278, 2, base, shadow],
      [_INTL("Color"), 326, 122, 2, base, shadow],
      [_INTL("Forma"), 454, 174, 2, base, shadow],
      [_INTL("Reiniciar"), 80, 346, 2, base, shadow, 1],
      [_INTL("Confirmar"), Graphics.width / 2, 346, 2, base, shadow, 1],
      [_INTL("Cancelar"), Graphics.width - 80, 346, 2, base, shadow, 1]
    ]
  ...
  def pbRefreshDexSearchParam(mode, cmds, sel, _index)
    ...
    textpos = [
      [_INTL("Modo de búsqueda"), Graphics.width / 2, 10, 2, base, shadow],
      [_INTL("Confirmar"), 80, 346, 2, base, shadow, 1],
    ...
    loop do
      ...
      if Input.trigger?(Input::UP)
        ...
        when 8   # Start search (filter)
          ...
          if dexlist.length == 0
            pbMessage(_INTL("No se han encontrado Pokémon coincidentes."))
          else
            @dexlist = dexlist
            @sprites["pokedex"].commands = @dexlist
