...
class PokemonPartyScreen
  ...
  def pbPokemonGiveMailScreen(mailIndex)
    ...
    if pkmnid >= 0
      ...
      if pkmn.hasItem? || pkmn.mail
        pbDisplay(_INTL("Este Pokémon sostiene un objeto. No puede llevar correo."))
      elsif pkmn.egg?
        pbDisplay(_INTL("Los huevos no pueden llevar correo."))
      else
        pbDisplay(_INTL("El correo se ha transferido desde el buzón."))
  ...
  def pbPokemonMultipleEntryScreenEx(ruleset)
    annot = []
    statuses = []
    ordinals = [_INTL("INELIGIBLE"), _INTL("NO INSCRITO"), _INTL("PROHIBIDO")]
    positions = [_INTL("PRIMERO"), _INTL("SEGUNDO"), _INTL("TERCERO"), _INTL("CUARTO"),
                 _INTL("QUINTO"), _INTL("SEXTO"), _INTL("SÉPTIMO"), _INTL("OCTAVO"),
                 _INTL("NOVENO"), _INTL("DÉCIMO"), _INTL("UNDÉCIMO"), _INTL("DUODÉCIMO")]
    Settings::MAX_PARTY_SIZE.times do |i|
    ...
    @scene.pbStartScene(@party, _INTL("Elige Pokémon y confirma."), annot, true)
    loop do
      realorder = []
      @party.length.times do |i|
      ...
      @scene.pbSetHelpText(_INTL("Elige Pokémon y confirma."))
      pkmnid = @scene.pbChoosePokemon
      addedEntry = false
      ...
      if (statuses[pkmnid] || 0) == 1
        commands[cmdEntry = commands.length]   = _INTL("Inscrito")
      elsif (statuses[pkmnid] || 0) > 2
        commands[cmdNoEntry = commands.length] = _INTL("No Inscrito")
      end
      pkmn = @party[pkmnid]
      ...
      if cmdEntry >= 0 && command == cmdEntry
        ...
        @scene.pbSummary(pkmnid) {
          @scene.pbSetHelpText((@party.length > 1) ? _INTL("Elige un Pokémon.") : _INTL("Choose Pokémon or cancel."))
        }
      end
    end
    @scene.pbEndScene
  ...
  def pbChooseTradablePokemon(ableProc, allowIneligible = false)
    ...
    loop do
      ...
      break if pkmnid < 0
      if !eligibility[pkmnid] && !allowIneligible
        pbDisplay(_INTL("No se puede elegir a este Pokémon."))
      else
        ret = pkmnid
        break
      end
  ...
  def pbPokemonScreen
    ...
    loop do
      ...
      if party_idx.is_a?(Array) && party_idx[0] == 1   # Switch
        @scene.pbSetHelpText(_INTL("¿Mover dónde?"))
        ...
        if [:MILKDRINK, :SOFTBOILED].include?(move.id)
          ...
          loop do
            ...
            if party_idx == old_party_idx
              ...
              pbDisplay(_INTL("¡{1} no se puede usar en un Huevo!", movename))
            elsif newpkmn.fainted? || newpkmn.hp == newpkmn.totalhp
              pbDisplay(_INTL("{1} no se puede usar en ese Pokémon.", movename))
            else
              pkmn.hp -= amt
              hpgain = pbItemRestoreHP(newpkmn, amt)
              @scene.pbDisplay(_INTL("Los PS de {1} se han restaurado en {2} puntos.", newpkmn.name, hpgain))
              pbRefresh
            end
...
MenuHandlers.add(:party_menu, :switch, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    screen.scene.pbSetHelpText(_INTL("¿Mover dónde?"))
    old_party_idx = party_idx
...
MenuHandlers.add(:party_menu, :mail, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    pkmn = party[party_idx]
    command = screen.scene.pbShowCommands(_INTL("¿Qué hacer con el correo?"),
  ...
  "effect"    => proc { |screen, party, party_idx|
    ...
    loop do
      ...
      if newpkmn.egg?
        ...
        screen.pbRefresh
        screen.pbDisplay(_INTL("Ahora {2} lleva un {1}.", newpkmn.name, itemname))
        moved = true
        break
      elsif newpkmn.item.is_mail?
        screen.pbDisplay(_INTL("Debes quitarle el correo a {1} antes de darle un objeto.", newpkmn.name))
        next
      end
      ...
      if newitem == :LEFTOVERS
        screen.pbDisplay(_INTL("{1} ya lleva {2}.\1", newpkmn.name, newitemname))
      elsif newitemname.starts_with_vowel?
        screen.pbDisplay(_INTL("{1} ya lleva {2}.\1", newpkmn.name, newitemname))
      else
        screen.pbDisplay(_INTL("{1} ya lleva {2}.\1", newpkmn.name, newitemname))
      end
      next if !screen.pbConfirm(_INTL("¿Quieres cambiar los dos objetos?"))
      newpkmn.item = item
      pkmn.item = newitem
      screen.scene.pbClearSwitching
      screen.pbRefresh
      screen.pbDisplay(_INTL("Ahora {2} lleva un {1}.", newpkmn.name, itemname))
      screen.pbDisplay(_INTL("Ahora {2} lleva un {1}.", pkmn.name, newitemname))
      moved = true
      break
    end
