...
class HallOfFame_Scene
  ...
  def writeTrainerData
    ...
    lefttext += _INTL("No. ID<r>{1}<br>", pubid)
    ...
    pbMessageDisplay(@sprites["msgwindow"],
                     _INTL("¡Has conquistado la Liga Pokémon!\n¡Enhorabuena!\\^"))
  end
  ...
  def writePokemonData(pokemon, hallNumber = -1)
    ...
    if hallNumber > -1
      textPositions.push([_INTL("Salón de la Fama No."), (Graphics.width / 2) - 104, 6, 0, BASECOLOR, SHADOWCOLOR])
  ...
  def writeWelcome
    overlay = @sprites["overlay"].bitmap
    overlay.clear
    pbDrawTextPositions(overlay, [[_INTL("¡Te damos la bienvenida al Salón de la Fama!"),
...
MenuHandlers.add(:pc_menu, :hall_of_fame, {
  "name"      => _INTL("Salón de la Fama"),
  "order"     => 40,
  "condition" => proc { next $PokemonGlobal.hallOfFameLastNumber > 0 },
  "effect"    => proc { |menu|
    pbMessage(_INTL("\\se[PC access]Has accedido al Salón de la Fama."))
    pbHallOfFamePC
    next false
  }
})
