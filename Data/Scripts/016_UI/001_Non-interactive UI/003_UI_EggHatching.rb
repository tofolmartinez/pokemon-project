...
class PokemonEggHatch_Scene
  ...
  def pbMain
    ...
    @pokemon.name = nil
    pbMessage(_INTL("\\se[]¡{1} ha salido del Huevo!\\wt[80]", @pokemon.name)) { update }
    ...
    if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned && $player.has_pokedex
      pbMessage(_INTL("Los datos de {1} se han registrado en la Pokédex.", @pokemon.name)) { update }
    ...
    if $PokemonSystem.givenicknames == 0 &&
       pbConfirmMessage(
         _INTL("¿Quieres ponerle un mote al recién nacido {1}?", @pokemon.name)
       ) { update }
...
def pbHatch(pokemon)
  ...
  if !pbHatchAnimation(pokemon)
    ...
    pbMessage(_INTL("...\1"))
    pbMessage(_INTL("... .... .....\1"))
    pbMessage(_INTL("¡{1} ha salido del Huevo!", speciesname))
    was_owned = $player.owned?(pokemon.species)
    $player.pokedex.register(pokemon)
    $player.pokedex.set_owned(pokemon.species)
    $player.pokedex.set_seen_egg(pokemon.species)
    # Show Pokédex entry for new species if it hasn't been owned before
    if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned && $player.has_pokedex
      pbMessage(_INTL("Los datos de {1} se han registrado en la Pokédex.", speciesname))
      $player.pokedex.register_last_seen(pokemon)
      pbFadeOutIn {
        scene = PokemonPokedexInfo_Scene.new
        screen = PokemonPokedexInfoScreen.new(scene)
    ...
    # Nickname the Pokémon
    if $PokemonSystem.givenicknames == 0 &&
       pbConfirmMessage(_INTL("¿Quieres ponerle un mote al recién nacido {1}?", speciesname))
