...
class PokemonPauseMenu
  ...
  def pbStartPokemonMenu
    if !$player
      if $DEBUG
        pbMessage(_INTL("No se ha definido el jugador, por lo que no se puede mostrar el menú de pausa."))
        pbMessage(_INTL("Consulte la documentación para aprender a configurar el jugador."))
      end
      return
    end
    @scene.pbStartScene
...
MenuHandlers.add(:pause_menu, :quit_game, {
  ...
  "effect"    => proc { |menu|
    menu.pbHideMenu
    if pbConfirmMessage(_INTL("¿Seguro que quieres salir del juego?"))
      scene = PokemonSave_Scene.new
      screen = PokemonSaveScreen.new(scene)
      screen.pbSaveScreen
      menu.pbEndScene
