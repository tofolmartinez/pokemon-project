...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    textpos = [
      ...
      [_INTL("Objeto"), 66, 324, 0, base, shadow]
    ]
  ...
  def drawPageOne
    ...
    if @pokemon.owner.name.empty?
      textpos.push([_INTL("ALQUILER"), 435, 182, 2, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    ...
    if @pokemon.shadowPokemon?
      textpos.push([_INTL("Corazón"), 238, 246, 0, base, shadow])
      heartmessage = [_INTL("¡La puerta a su corazón se ha abierto! ¡Rompe el último cierre!"),
                      _INTL("La puerta a su corazón está casi completamente abierta."),
                      _INTL("La puerta a su corazón se está abriendo cada vez más."),
                      _INTL("La puerta a su corazón está entreabierta."),
                      _INTL("La puerta a su corazón ha empezado a abrirse."),
                      _INTL("La puerta a su corazón está firmemente cerrada.")][@pokemon.heartStage]
      memo = sprintf("<c3=404040,B0B0B0>%s\n", heartmessage)
      drawFormattedTextEx(overlay, 234, 308, 264, memo)
    else
      endexp = @pokemon.growth_rate.minimum_exp_for_level(@pokemon.level + 1)
      textpos.push([_INTL("Puntos Exp."), 238, 246, 0, base, shadow])
      textpos.push([@pokemon.exp.to_s_formatted, 488, 278, 1, Color.new(64, 64, 64), Color.new(176, 176, 176)])
      textpos.push([_INTL("Siguiente Nv."), 238, 310, 0, base, shadow])
  ...
  def drawPageOneEgg
    ...
    # Write various bits of text
    textpos = [
      [_INTL("NOTAS ENTRENADOR"), 26, 22, 0, base, shadow],
      [@pokemon.name, 46, 68, 0, base, shadow],
      [_INTL("Objeto"), 66, 324, 0, base, shadow]
    ]
    # Write the held item's name
    ...
    if mapname && mapname != ""
      memo += _INTL("<c3=404040,B0B0B0>Un misterioso Huevo Pokémon recibido desde <c3=F83820,E09890>{1}<c3=404040,B0B0B0>.\n", mapname)
    else
    ...
    eggstate = _INTL("Parece que a este Huevo aún le falta para eclosionar.")
    eggstate = _INTL("¿Qué saldrá de aquí? No parece que vaya a abrirse.") if @pokemon.steps_to_hatch < 10_200
    eggstate = _INTL("Se mueve a veces. Eclosionará pronto.") if @pokemon.steps_to_hatch < 2550
    eggstate = _INTL("¡Pueden oírse sonidos del interior! ¡Le queda muy poquito!") if @pokemon.steps_to_hatch < 1275
    memo += sprintf("<c3=404040,B0B0B0>%s\n", eggstate)
    # Draw all text
  ...
  def drawPageTwo
    ...
    mettext = [_INTL("Encontrado al Nv. {1}.", @pokemon.obtain_level),
               _INTL("Huevo recibido."),
               _INTL("Intercambiado al nv. {1}.", @pokemon.obtain_level),
               "",
               _INTL("Tuvo un encuentro predestinado al Nv. {1}.", @pokemon.obtain_level)][@pokemon.obtain_method]
  ...
  def drawPageFourSelecting(move_to_learn)
    ...
    # Write various bits of text
    textpos = [
      [_INTL("MOVIMIENTOS"), 26, 22, 0, base, shadow],
      [_INTL("CATEGORÍA"), 20, 128, 0, base, shadow],
      [_INTL("PODER"), 20, 160, 0, base, shadow],
      [_INTL("PRECISIÓN"), 20, 192, 0, base, shadow]
    ]
    imagepos = []
    # Write move names, types and PP amounts for each known move
    yPos = 104
  ...
  def drawPageFive
    ...
    # Write various bits of text
    textpos = [
      [_INTL("No. de cintas:"), 234, 338, 0, Color.new(64, 64, 64), Color.new(176, 176, 176)],
  ...
  def pbMarking(pokemon)
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
          [_INTL("Seleccionar {1}", pokemon.name), 366, 102, 2, base, shadow],
          [_INTL("OK"), 366, 254, 2, base, shadow],
          [_INTL("Cancelar"), 366, 304, 2, base, shadow]
        ]
  ...
  def pbOptions
    ...
    if !@pokemon.egg?
      ...
      commands[cmdTakeItem = commands.length] = _INTL("Quitar objeto") if @pokemon.hasItem?
      commands[cmdPokedex = commands.length]  = _INTL("Ver Pokédex") if $player.has_pokedex
    end
    commands[cmdMark = commands.length]       = _INTL("Seleccionar")
...
class PokemonSummaryScreen
  ...
  def pbStartForgetScreen(party, partyindex, move_to_learn)
    ...
    loop do
      ...
      pbMessage(_INTL("Los movimientos MO no pueden ser olvidados ahora.")) { @scene.pbUpdate }
    end
    @scene.pbEndScene
    return ret
  end
