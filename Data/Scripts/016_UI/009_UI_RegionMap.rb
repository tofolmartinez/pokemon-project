...
class PokemonRegionMap_Scene
  ...
  def pbStartScene(as_editor = false, fly_map = false)
    ...
    if !@map
      pbMessage(_INTL("No se han encontrado los datos del mapa."))
      return false
    end
  ...
  def pbChangeMapLocation(x, y)
    ...
    currentname = pbMessageFreeText(_INTL("Introduce el nombre para este punto."), currentname, false, 250) { pbUpdate }
    if currentname
      if currentobj
