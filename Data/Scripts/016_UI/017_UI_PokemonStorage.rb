...
class PokemonBoxPartySprite < Sprite
  ...
  def refresh
    ...
    pbDrawTextPositions(
      self.bitmap,
      [[_INTL("Atrás"), 86, 248, 2, Color.new(248, 248, 248), Color.new(80, 80, 80), 1]]
    )
...
class PokemonStorageScene
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
          ...
          [_INTL("Cancelar"), 402, 280, 2, base, shadow, 1]
        ]
  ...
  def pbUpdateOverlay(selection, party = nil)
    ...
    pbDrawTextPositions(
      overlay,
      [[_INTL("Equipo: {1}", (@storage.party.length rescue 0)), 270, 334, 2, buttonbase, buttonshadow, 1],
       [_INTL("Salir"), 446, 334, 2, buttonbase, buttonshadow, 1]]
    )
    pokemon = nil
    ...
    if !pokemon.egg?
      ...
      if pokemon.ability
        ...
        textstrings.push([_INTL("Sin habilidad"), 86, 312, 2, nonbase, nonshadow])
      end
      if pokemon.item
        textstrings.push([pokemon.item.name, 86, 348, 2, base, shadow])
      else
        textstrings.push([_INTL("Sin objeto"), 86, 348, 2, nonbase, nonshadow])
      end
      if pokemon.shiny?
