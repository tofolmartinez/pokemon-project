...
class PokemonBagScreen
  ...
  def pbStartScreen
    ...
    loop do
      ...
      itemname = itm.name
      command = @scene.pbShowCommands(_INTL("{1} ha sido seleccionado.", itemname), commands)
      if cmdRead >= 0 && command == cmdRead   # Read mail
        ...
        if $player.pokemon_count == 0
          ...
          @scene.pbDisplay(_INTL("No puedes dar {1}.", itemname))
        else
          pbFadeOutIn {
        ...
        if qty > 1
          helptext = _INTL("¿Qué cantidad de {1} quieres tirar?", itm.name_plural)
        ...
        command = 0
        loop do
          command = @scene.pbShowCommands(_INTL("¿Que quieres hacer con {1}?", itemname),
                                          [_INTL("Cambiar cantidad"),
                                           _INTL("Crear Regalo Misterioso"),
          ...
          when 0
            ...
            newqty = pbMessageChooseNumber(
              _INTL("Elige la nueva cantidad de {1} (max. #{Settings::BAG_MAX_PER_SLOT}).", itemplural), params
            ) { @scene.pbUpdate }
            if newqty > qty
  ...
  def pbWithdrawItemScreen
    ...
    loop do
      ...
      if qty > 1 && !itm.is_important?
        qty = @scene.pbChooseNumber(_INTL("¿Qué cantidad quieres retirar?"), qty)
      end
      next if qty <= 0
      if @bag.can_add?(item, qty)
        if !storage.remove(item, qty)
          raise "No puedes borrar objetos del almacenamiento"
        end
        if !@bag.add(item, qty)
          raise "No puedes retirar objetos del almacenamiento"
        end
        @scene.pbRefresh
        dispqty = (itm.is_important?) ? 1 : qty
        itemname = (dispqty > 1) ? itm.name_plural : itm.name
        pbDisplay(_INTL("Retiraste {1} {2}.", dispqty, itemname))
      else
        pbDisplay(_INTL("No hay más espacio en la bolsa."))
      end
    end
    @scene.pbEndScene
  end
  ...
  def pbDepositItemScreen
    ...
    loop do
      ...
      qty = @bag.quantity(item)
      if qty > 1 && !itm.is_important?
        qty = @scene.pbChooseNumber(_INTL("¿Qué cantidad quieres depositar?"), qty)
      end
      if qty > 0
        if storage.can_add?(item, qty)
          if !@bag.remove(item, qty)
            raise "No puedes borrar objetos de la Bolsa"
          end
          if !storage.add(item, qty)
            raise "No puedes depositar objetos en el almacenamiento"
          end
          @scene.pbRefresh
          dispqty  = (itm.is_important?) ? 1 : qty
          itemname = (dispqty > 1) ? itm.name_plural : itm.name
          pbDisplay(_INTL("Has depositado {1} {2}.", dispqty, itemname))
        else
  ...
  def pbTossItemScreen
    ...
    loop do
      ...
      itm = GameData::Item.get(item)
      if itm.is_important?
        @scene.pbDisplay(_INTL("¡Esto es demasiado importante para tirarlo!"))
        next
      end
      qty = storage.quantity(item)
      itemname       = itm.name
      ...
      if qty > 1
        qty = @scene.pbChooseNumber(_INTL("¿Qué cantidad de {1} quieres tirar?", itemnameplural), qty)
      end
      next if qty <= 0
