...
class Window_PokemonMart < Window_DrawableCommand
  ...
  def drawItem(index, count, rect)
    ...
    ypos = rect.y
    if index == count - 1
      textpos.push([_INTL("CANCELAR"), rect.x, ypos + 2, false, self.baseColor, self.shadowColor])
    else
      item = @stock[index]
...
class PokemonMart_Scene
  ...
  def pbRefresh
    if @subscene
      ...
      @sprites["qtywindow"].text    = _INTL("En la Bolsa:<r>{1}", @adapter.getQuantity(itemwindow.item))
  ...
  def pbStartBuyOrSellScene(buying, stock, adapter)
    ...
    @sprites["qtywindow"].text = _INTL("En la Bolsa:<r>{1}", @adapter.getQuantity(@sprites["itemwindow"].item))
    @sprites["qtywindow"].y    = Graphics.height - 102 - @sprites["qtywindow"].height
    ...
    loop do
      ...
      if GameData::Item.get(item).is_important?
        ...
        quantity = @scene.pbChooseNumber(
          _INTL("¿Qué cantidad de {1} quieres?", itemnameplural), item, maxafford
        )
        next if quantity == 0
        price *= quantity
        if quantity > 1
          next if !pbConfirm(_INTL("Así que {1} {2}?\nEso serán ${3}. ¿Te parece bien?",
      ...
      if added == quantity
        ...
        pbDisplayPaused(_INTL("¡Aquí tienes! ¡Gracias!")) { pbSEPlay("Mart buy item") }
        if quantity >= 10 && GameData::Item.exists?(:PREMIERBALL)
          if Settings::MORE_BONUS_PREMIER_BALLS && GameData::Item.get(item).is_poke_ball?
            ...
            pbDisplayPaused(_INTL("¡Y ten {1} {2} de regalo!", premier_balls_added, ball_name))
            ...
            if @adapter.addItem(:PREMIERBALL)
              ...
              pbDisplayPaused(_INTL("Y ten 1 {1} de regalo!", ball_name))
            end
          end
        end
      else
        added.times do
          if !@adapter.removeItem(item)
            raise _INTL("Fallo al borrar objetos guardados")
          end
        end
        pbDisplayPaused(_INTL("No tienes espacio en la Bolsa."))
      end
    end
    @scene.pbEndBuyScene
  end
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if pbConfirm(_INTL("Puedo pagarte ${1}.\n¿Te parece bien?", price.to_s_formatted))
        ...
        pbDisplayPaused(_INTL("Vendes {1} y recibes ${2}.",
...
def pbPokemonMart(stock, speech = nil, cantsell = false)
  ...
  cmd = pbMessage(speech || _INTL("¡Hola! ¿En qué puedo ayudarte?"), commands, cmdQuit + 1)
  loop do
    ...
    cmd = pbMessage(_INTL("¿Puedo hacer algo más por ti"), commands, cmdQuit + 1)
  end
