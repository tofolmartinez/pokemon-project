...
class MoveRelearner_Scene
  ...
  def pbDrawMoveList
    ...
    textpos = [
      [_INTL("¿Qué movimiento quieres enseñar?"), 16, 14, 0, Color.new(88, 88, 80), Color.new(168, 184, 184)]
    ]
    imagepos = []
    yPos = 88
    ...
    textpos.push([_INTL("CATEGORÍA"), 272, 120, 0, Color.new(248, 248, 248), Color.new(0, 0, 0)])
    textpos.push([_INTL("PODER"), 272, 152, 0, Color.new(248, 248, 248), Color.new(0, 0, 0)])
    textpos.push([basedamage <= 1 ? basedamage == 1 ? "???" : "---" : sprintf("%d", basedamage),
                  468, 152, 2, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    textpos.push([_INTL("PRECISIÓN"), 272, 184, 0, Color.new(248, 248, 248), Color.new(0, 0, 0)])
...
class MoveRelearnerScreen
  ...
  def pbStartScreen(pkmn)
    ...
    loop do
      ...
      elsif @scene.pbConfirm(_INTL("¿Dejar de enseñar un nuevo movimiento a {1}?", pkmn.name))
        @scene.pbEndScene
        return false
      end
    end
